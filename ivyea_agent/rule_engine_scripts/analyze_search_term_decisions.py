...
try:
    # Optional: only needed when --with-listing-context is used. Avoids pulling
    # bs4/playwright into Ivyea Agent's P1 (read-only patrol doesn't fetch listings).
    from fetch_listing_context import fetch_listing_context  # type: ignore
except Exception:  # noqa: BLE001
    fetch_listing_context = None  # type: ignore
...
def load_listing_context_live(asin: str, site_code: str) -> tuple[str | None, str | None, str | None]:
    if fetch_listing_context is None:
