...
> **stdio server 的环境变量**：子进程默认能读到本机全部环境变量（含各家 API key）。
> 想收紧不用改配置文件，用命令：
> ```bash
> ivyea mcp doctor                          # 看谁还能读到全部
> ivyea mcp env 某服务器 --secure            # 收紧到白名单（PATH/HOME/LANG/TZ/TEMP…）
> ivyea mcp env 某服务器 --pass GITHUB_TOKEN  # 再放行它确实需要的
