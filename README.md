...
- 最新 Release：`v1.2.0`（main 分支可能包含尚未打包的新改动）
...
```bash
curl -fsSL https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.sh | IVYEA_VERSION=v1.2.0 bash
```

```powershell
$env:IVYEA_VERSION="v1.2.0"
iwr https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.ps1 -UseBasicParsing | iex
```
...
## 对话里就能用：代码 / 子 agent / MCP（v1.0.21–v1.0.22）

上面这些原来只在 CLI，现在**对话式 agent 直接当工具用**，对标 Claude Code / Codex 的交互：

- **代码导航**：`grep`（内容正则）、`code_search`（找相关文件）、`code_symbols`、`code_impact`（改动影响面）——先定位再读，不瞎猜路径。
- **结构化改代码闭环**：`code_apply_patch`（默认 dry-run 校验，`execute=true` 才写并自动跑测试，经人工审批）→ `run_tests` → `code_repair`。
- **子 agent**：`dispatch_subagent` 派一个**只读、受限步数、不能递归**的子 agent 并行铺开多角度调研，结论回主线，不污染主上下文。
- **并行只读**：一步里多个只读工具（读文件/grep/web 抓取）并发执行，降延迟；写/审批类仍顺序保证语义。
- **轮内防溢出**：长任务在步边界估算 token，越硬上限自动压缩，避免上下文撑爆请求。
- **沙箱加固**：`run_command` / `run_python` 在 *nix 上加资源限额（内存/CPU/文件/禁 core），叠加既有的 cwd + 超时 + 危险命令拦截 + 人工审批。
- **可扩展**：`~/.ivyea/commands/<名字>.md` 即自定义斜杠命令（`$ARGUMENTS` 占位）；`~/.ivyea/hooks.json` 在 `user_prompt`/`session_start`/`session_end`/`stop`/`pre_tool_use`/`post_tool_use` 上跑钩子命令（opt-in，零配置零开销）。`pre_tool_use` 是决策钩子：条目可写 `{"matcher": "run_command|write_file", "command": "guard.sh"}`（matcher 为工具名正则），命令 exit 2（stderr 作理由）或输出 `{"decision":"block","reason":"..."}` 即拒绝该工具调用；其余情况一律放行（fail-open）。

## Headless / 无人值守升级（v1.2.0）

对标 Claude Code headless 模式的一批升级，`-p` 一次性运行现在可以当真正的程序化 runner 用：

- **结构化输出**：`ivyea chat -p "..." --output-format stream-json` 逐行输出 NDJSON 事件（`system/init` → `assistant`(text/tool_use) → `user`(tool_result) → `result`），字段对齐 Claude Code（费用为 `total_cost_cny`），程序侧可直接做工具调用可视化。
- **会话续接**：`-p` 跑完会落盘会话，`-p --resume <session_id>` 真续接多轮对话（此前 `-p` 只读不写会话）。
- **审批档位**：`--permission-mode {default,policy,approve-all}`——`policy` 档无人值守下按 `~/.ivyea/policy.json` 自动判定：命令走 allow/deny + 风险评估、文件写走写根校验、其余写类（广告执行/领星写等）一律拒绝；单工具拒绝**不终止整轮**。`--approve-all` 保留为全放行别名。
- **压缩保近期**：自动压缩保留最近 `compact_keep_recent`（默认 6）条消息原文、只摘要更旧的历史——压缩后不"失忆"刚读过的文件和路径。
- **视觉旁路**：主脑不支持图片（如 DeepSeek）时，`@图片.png` 自动由已配 key 的视觉模型（openai/anthropic/gemini；`ivyea config set vision_model <id>` 可指定）代读，分析文本回灌主线后继续。
- **并行子 agent**：一步派多个 `dispatch_subagent` 真并发铺开调研（此前排队串行）。
- **长输出可续读**：`run_command` 超长输出被截断时全量落盘 `~/.ivyea/outputs/<会话>/`，模型可 `read_file` 分段续读。

**MCP 也进了对话**（v1.0.22）：

- 发现/调用任意已配置的 MCP server：`mcp_list_tools` / `mcp_call_tool` / `mcp_list_resources` / `mcp_read_resource` / `mcp_list_prompts` / `mcp_get_prompt`。协议覆盖 **tools + resources + prompts**（http / sse / stdio 传输）。
- `mcp_call_tool` 门控：**计划模式拒绝**；`mcp.json` 里给某 server 标 `"trusted": true` 则免审，否则逐次人工审批。只读的 list/read/get 自动放行、也可被子 agent 使用。
- 反过来 ivyea 作为 **MCP server**（`ivyea mcp serve`）也升级了：把**知识卡暴露为 resources**（`ivyea-knowledge://<id>`）、**Skills 暴露为 prompts**，别的 MCP 客户端（IvyeaOps / Claude 等）接上即可用，不只是 tools。
...
工程类对话会显示 `Code 计划 → 读上下文 → 修改/生成补丁 → 测试 → 复查` 阶段提示；工具调用会带 `1/48.1` 这样的进度编号，方便判断当前卡在读文件、跑测试还是生成补丁。终端默认使用带框输入区：输入框始终钉在底部，提交后输入会回显成一行历史，对话自上而下在框上方累积（与 Claude Code / Codex 一致）；想退回轻量单行输入可设置 `IVYEA_BOXED_INPUT=0`，不想要颜色可设置 `NO_COLOR=1`。
...
| 变量 | 用途 |
| --- | --- |
| `IVYEA_VERSION=v1.2.0` | 固定安装某个 Release |
...
配置写入 `~/.ivyea/mcp.json`（权限 600，密钥只在你本机）。客户端是**通用的**——不绑死任何厂商工具名，由你配的 `dataSource` 映射驱动（approach c）。传输支持 `http`、`sse` 和本地 `stdio`，协议覆盖 **tools + resources + prompts**。`ivyea mcp doctor` 会检查 transport、dataSource、writeActions、安全风险（例如带鉴权的明文 HTTP、stdio 命令缺失、疑似明文 token），并提示真实写入仍必须显式 `--execute` 且走审批/审计。

> **对话里直接用（v1.0.22）**：除了 CLI，agent 在 `ivyea chat` 里能用 `mcp_list_tools` / `mcp_call_tool` / `mcp_list_resources` / `mcp_read_resource` / `mcp_list_prompts` / `mcp_get_prompt` 自主发现并调用你接的任意 MCP server。`mcp_call_tool` 计划模式拒绝、给某 server 在 mcp.json 标 `"trusted": true` 则免审、否则逐次审批；只读的 list/read/get 自动放行。

反向 MCP（`ivyea mcp serve`）默认只暴露只读能力：**tools**（健康状态、manifest、知识检索、统一检索、skill 搜索、system doctor、task list/detail/resume、trace list/stats、workspace 搜索/巡检、code plan/context/bundle/repair）、**resources**（知识卡，`ivyea-knowledge://<id>`）和 **prompts**（Skills）。它不暴露 `execute_actions`、文件写入或命令执行。
...
**工程化**：pytest 覆盖规则引擎、写入护栏、skills、知识库、trace、安全脱敏、画像、业务 eval；`ivyea eval` 固化关键质量回归（广告决策 golden、知识召回、skill 召回、安全脱敏）+ ruff lint + **GitHub Actions CI**（ubuntu/macos/windows × py3.9/3.11/3.12，三平台全绿；测试命令执行已做跨平台守卫，不再依赖 *nix 专有 `bash -lc`）+ **打 tag 即发布**（构建 sdist、wheel 和离线部署包挂到 GitHub Release）。当前文档按 **v1.2.0** 示例维护。
