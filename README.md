...
| 变量 | 用途 |
| --- | --- |
| `IVYEA_VERSION=v0.5.3` | 固定安装某个 Release |
...
`workspace` 会离线扫描文本文件，跳过 `.git/node_modules/dist/build/venv` 等噪声目录，索引保存到 `~/.ivyea/workspaces/`。它能输出语言分布、重要文件、目录结构、符号摘要和搜索命中，是后续代码理解、任务执行和 Review 的地基。
...
ivyea gitops release-plan --root . --version v0.5.3
...
ivyea gitops tag --root . --tag v0.5.3 --execute
ivyea codereview --root .
```

`gitops` 提供仓库状态、diff 摘要、GitHub Actions workflow 发现、发版前检查，以及本地 `stage/commit/tag` 写操作。写操作默认只输出预览，`--execute` 才执行；不加 `--yes` 会弹人工审批。`codereview` 会扫描 git diff（含未跟踪新文件），识别疑似密钥、危险 shell、宽异常捕获、生产代码无测试改动等确定性风险。
...
用户 skill 存在 `~/.ivyea/skills/`，可以覆盖或扩展内置 Amazon skills。`skill audit` 会检查触发词、正文和关联知识卡是否缺失。
...
**通用工程 Agent 能力**：已新增 `workspace`（本地项目索引/搜索/地图/解释）、`task`（长任务状态机/步骤/日志/resume）、`patch`（结构化补丁校验/应用/测试建议）、`gitops`（Git 状态/diff/workflow/发版检查/stage/commit/tag）、`codereview`（确定性 diff 风险审查）、`skill create/audit`（用户 skill 脚手架/审计）和通用 `vision`（截图/UI/报表多模态入口）。这些是对标 Codex/Claude Code 的通用工程底座，写操作默认 dry-run 并经过人工审批。
...
**工程化**：pytest 覆盖规则引擎、写入护栏、skills、知识库、trace、安全脱敏、画像、业务 eval；`ivyea eval` 固化关键质量回归（广告决策 golden、知识召回、skill 召回、安全脱敏）+ ruff lint + **GitHub Actions CI**（ubuntu/macos/windows × py3.9/3.11/3.12）+ **打 tag 即发布**（构建 sdist、wheel 和离线部署包挂到 GitHub Release）。已发 **v0.5.3**。
