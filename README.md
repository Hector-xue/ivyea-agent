...
- 最新 Release：`v1.0.44`（main 分支可能包含尚未打包的新改动）
...
```bash
curl -fsSL https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.sh | IVYEA_VERSION=v1.0.44 bash
```

```powershell
$env:IVYEA_VERSION="v1.0.44"
iwr https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.ps1 -UseBasicParsing | iex
```
...
| 变量 | 用途 |
| --- | --- |
| `IVYEA_VERSION=v1.0.44` | 固定安装某个 Release |
...
**工程化**：pytest 覆盖规则引擎、写入护栏、skills、知识库、trace、安全脱敏、画像、业务 eval；`ivyea eval` 固化关键质量回归（广告决策 golden、知识召回、skill 召回、安全脱敏）+ ruff lint + **GitHub Actions CI**（ubuntu/macos/windows × py3.9/3.11/3.12，三平台全绿；测试命令执行已做跨平台守卫，不再依赖 *nix 专有 `bash -lc`）+ **打 tag 即发布**（构建 sdist、wheel 和离线部署包挂到 GitHub Release）。当前文档按 **v1.0.44** 示例维护。
