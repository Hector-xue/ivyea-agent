...
**对话式内核**：**流式输出 + 多步工具循环 + 成本/token 核算 + 计划模式**已实现并对真实 DeepSeek 验证通过（含 prompt caching 实测命中）。聊天里：`/plan` 进只读计划模式、`/approve` 批准执行、`/cost` 看用量、状态栏实时显示累计 ¥。

**尚未实现 / 待办**（不再标“✅完成”）：
- 领星**真实写入**仅 dry-run/单测验证，**尚未在生产真按下写**（需活跃广告店 + 你授权开 operate 实测）。
- 上下文压缩（`/compact`）；会话 resume。
- Anthropic 原生 + prompt caching；Gemini/登录制（DeepSeek caching 已自动生效）。
- 记忆摘要压缩 / 会话转录回忆 / 自策展。
- 终端 Markdown/Diff/Todo 渲染。

路线（总纲 M0–M7）：M0 止血+核实 ✅ → M1 领星适配层（只读）✅ → M2 领星写入执行+审批 ✅ → M1+ 内核(流式/成本/Plan) ✅ → 上下文压缩/resume → 模型层 → 工具能力 → 记忆 → 交互/视觉 → 工程化 → 嵌入 IvyeaOps。
