...
路线（总纲 M0–M7）：M0 止血+核实 ✅ → M1 领星适配层（只读）✅ → M2 领星写入执行+审批 ✅ → M1+ 内核(流式/成本/Plan) ✅ → M1++ 上下文压缩/resume/Markdown ✅ → 模型层(Claude 原生+caching) ✅ → 工具能力(文件/执行/web+沙箱+门控) ✅ → 记忆 → 交互/视觉 → 工程化 → 嵌入 IvyeaOps。
