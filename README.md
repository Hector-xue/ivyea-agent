...
```bash
curl -fsSL https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.sh | IVYEA_VERSION=v0.5.2 bash
```

```powershell
$env:IVYEA_VERSION="v0.5.2"
iwr https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.ps1 -UseBasicParsing | iex
```
...
| 变量 | 用途 |
| --- | --- |
| `IVYEA_VERSION=v0.5.2` | 固定安装某个 Release |
...
ivyea gitops release-plan --root . --version v0.5.2
ivyea codereview --root .
```

`gitops` 当前只做只读检查：仓库状态、diff 摘要、GitHub Actions workflow 发现、发版前检查（工作区是否干净、版本号是否匹配、tag 是否已存在、release workflow 是否存在）。`codereview` 会扫描 git diff（含未跟踪新文件），识别疑似密钥、危险 shell、宽异常捕获、生产代码无测试改动等确定性风险。真实 commit/push/release 后续会接审批和登录态。
...
**通用工程 Agent 能力**：已新增 `workspace`（本地项目索引/搜索/地图/解释）、`task`（长任务状态机/步骤/日志/resume）、`gitops`（Git 状态/diff/workflow/发版检查）、`codereview`（确定性 diff 风险审查）、`skill create/audit`（用户 skill 脚手架/审计）和通用 `vision`（截图/UI/报表多模态入口）。这些是对标 Codex/Claude Code 的通用工程底座，当前以只读/本地状态和请求包生成为主，后续接补丁生成和带审批的 Git 写操作。
...
**工程化**：pytest 覆盖规则引擎、写入护栏、skills、知识库、trace、安全脱敏、画像、业务 eval；`ivyea eval` 固化关键质量回归（广告决策 golden、知识召回、skill 召回、安全脱敏）+ ruff lint + **GitHub Actions CI**（ubuntu/macos/windows × py3.9/3.11/3.12）+ **打 tag 即发布**（构建 sdist、wheel 和离线部署包挂到 GitHub Release）。已发 **v0.5.2**。
