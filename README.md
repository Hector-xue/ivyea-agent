...
```bash
curl -fsSL https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.sh | IVYEA_VERSION=v0.5.4 bash
```

```powershell
$env:IVYEA_VERSION="v0.5.4"
iwr https://raw.githubusercontent.com/Hector-xue/ivyea-agent/main/scripts/install.ps1 -UseBasicParsing | iex
```
...
| 变量 | 用途 |
| --- | --- |
| `IVYEA_VERSION=v0.5.4` | 固定安装某个 Release |
...
ivyea gitops release-plan --root . --version v0.5.4
...
ivyea gitops tag --root . --tag v0.5.4 --execute
...
**通用工程 Agent 能力**：已新增 `workspace`（本地项目索引/搜索/地图/解释）、`task`（长任务状态机/步骤/日志/resume）、`patch`（结构化补丁校验/应用/测试建议）、`gitops`（Git 状态/diff/workflow/CI 状态/发版检查/stage/commit/tag）、`codereview`（确定性 diff 风险审查）、`skill create/audit`（用户 skill 脚手架/审计）和通用 `vision`（截图/UI/报表多模态入口）。这些是对标 Codex/Claude Code 的通用工程底座，写操作默认 dry-run 并经过人工审批。
...
**工程化**：pytest 覆盖规则引擎、写入护栏、skills、知识库、trace、安全脱敏、画像、业务 eval；`ivyea eval` 固化关键质量回归（广告决策 golden、知识召回、skill 召回、安全脱敏）+ ruff lint + **GitHub Actions CI**（ubuntu/macos/windows × py3.9/3.11/3.12）+ **打 tag 即发布**（构建 sdist、wheel 和离线部署包挂到 GitHub Release）。已发 **v0.5.4**。
